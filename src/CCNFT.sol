...
contract CCNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("CryptoCampo", "CCNFT") {}
    ...
    function buy(uint256 value, uint256 amount) external nonReentrant {
        require(canBuy, "Buying is not allowed"); // Verificación de permisos de la compra con "canBuy". Incluir un mensaje de falla.
        ...
        for (
            uint256 i = 0; // Bucle desde 1 hasta amount (inclusive) para mintear la cantidad especificada de NFTs.
            i < amount;
            i++
        ) {
    ...
    function setFundsCollector(address collector) external onlyOwner {
        // Parámetro, dirección de colector de fondos.
        require(collector != address(0), "Invalid address"); // La dirección no puede ser la dirección cero (address(0))
        fundsCollector = collector; // Dirección proporcionada a la variable fundsCollector.
    }

    // Dirección para colectar las tarifas de transacción.
    function setFeesCollector(address collector) external onlyOwner {
        // Parámetro, dirección del colector de tarifas.
        require(collector != address(0), "Invalid address"); // La dirección no puede ser la dirección cero (address(0))
        feesCollector = collector; // Dirección proporcionada a la variable feesCollector.
    }
    ...
    function addToArray(uint256[] storage _list, uint256 id) private {
    ...
    function find(uint256[] storage _list, uint256 id) private view returns (uint256) {
        ...
        return _list.length; // Si no se encuentra, retornar la longitud del array.
    }
